function pocetnaStrana() {
   ...
   $("#divRegForm").submit((e) => {
      ...
      if (error_fname === false && error_sname === false && error_email === false && error_password === false && error_retype_password === false) {
         ...
         userStudentReg.ime = corr_fname;
         userStudentReg.prezime = corr_sname;
         userStudentReg.meil = corr_email;
         userStudentReg.passw = corr_password;
         userStudentReg.role = "student";
         userStudentReg.books = [];

         var lsStudents = JSON.parse(localStorage.getItem("userStudent"));

         if (lsStudents.length == 0) {
            lsStudents.push(userStudentReg);
            localStorage.setItem("userStudent", JSON.stringify(lsStudents));
            alert("Registration Successful");
         } else {
            for (var i = 0; i < lsStudents.length; i++) {
               if (lsStudents[i].meil == corr_email) {
                  flag = true;
                  break;
               }
            }
            ...
            if (flag) {
               alert("This account alredy exist...log in!!!");
            } else {
               lsStudents.push(userStudentReg);
               localStorage.setItem("userStudent", JSON.stringify(lsStudents));
   ...
   $("#divLogForm").submit((e) => {
      ...
      if (error_email === false && error_password === false) {
         // alert("Registration Successfull");
         //da se proverat so local storage
         var lsAll = [];
         var lsStudents = JSON.parse(localStorage.getItem("userStudent"));
         var lsLib = JSON.parse(localStorage.getItem("userLib"));
         if (lsStudents == null) {
            var lsAll = lsLib;
         } else {
            //  alert("ima studenti");
            var lsAll = lsLib.concat(lsStudents);
         }
         // console.log(lsAll.length);
         // alert(lsAll[3].meil);
         var flag = false;
         for (var i = 0; i < lsAll.length; i++) {
            //  console.log(lsAll[i].meil + " / " + corr_email_1);
            //  console.log(lsAll[i].passw +" / "+ corr_password_1);
            //  alert(i);
            if (lsAll[i].meil == corr_email_1 && lsAll[i].passw == corr_password_1) {
               alert("Login Successfull");
               flag = true;
               var userType = lsAll[i].role;
               console.log(userType);
               if (userType == "student") {
                  sessionStorage.setItem('ime', lsAll[i].ime);
                  sessionStorage.setItem('prezime', lsAll[i].prezime)
                  sessionStorage.setItem('emeil', lsAll[i].meil)
